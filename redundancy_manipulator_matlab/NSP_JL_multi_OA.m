...
lightFixture1 = collisionSphere(0.075);
lightFixture1.Pose = trvec2tform([0.1 -0.65 0.7]);

lightFixture2 = collisionSphere(0.075);
lightFixture2.Pose = trvec2tform([0.1 -0.65 0.7]);
% lightFixture2.Pose = trvec2tform([-0.3 -0.3 0.4]);
...
% H function & Gradient H
H0_JL = performance_criterion_JL(joint_limit, WLN_q(:,1));
init_g_H0_JL = get_gradient_H_JL(joint_limit, WLN_q(:,1));
H1_OA = performance_criterion_OA(sepDist(:,1));
init_g_H1_OA = [0 0 0 0 0 0 0];
H2_OA = performance_criterion_OA(sepDist(:,2));
init_g_H2_OA = [0 0 0 0 0 0 0];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% parameters
W=eye(7,7);
k0_JL= -0.00000;
k1_OA=0.005; % to be maximized
k2_OA=0.005;

% initial calculation(joint velocity)
gen_J_pinv=inv(W)*transpose(J)*inv(J*inv(W)*transpose(J));
WLN_q_dot(:,1) = gen_J_pinv*x_dot + k0_JL*(eye(7,7) - gen_J_pinv*J)*transpose(init_g_H0_JL) + k1_OA*(eye(7,7) - gen_J_pinv*J)*transpose(init_g_H1_OA) + k2_OA*(eye(7,7) - gen_J_pinv*J)*transpose(init_g_H2_OA);
% disp(size(gen_J_pinv*x_dot))
% disp(size(k0_JL*(eye(7,7) - gen_J_pinv*J)*transpose(init_g_H0_JL)))
NSP(:,1)=[gen_J_pinv*x_dot; k0_JL*(eye(7,7) - gen_J_pinv*J)*transpose(init_g_H0_JL) ; k1_OA*(eye(7,7) - gen_J_pinv*J)*transpose(init_g_H1_OA) ; k2_OA*(eye(7,7) - gen_J_pinv*J)*transpose(init_g_H2_OA)];

now_g_H1_OA = init_g_H1_OA;
now_g_H2_OA = init_g_H2_OA;
%%%%%%%%%%%% next_step %%%%%%%%%%%%
for i=2:size(pos_joint,1)
    ...
    % Update H, g_H of "JL"
    H0_JL = performance_criterion_JL(joint_limit, WLN_q(:,i-1));
    now_g_H0_JL = get_gradient_H_JL(joint_limit, WLN_q(:,i-1));

    % WLN definition
    WLN_q_dot(:,i) = gen_J_pinv*x_dot + k0_JL*(eye(7,7) - gen_J_pinv*J)*transpose(now_g_H0_JL) + k1_OA*(eye(7,7) - gen_J_pinv*J)*transpose(now_g_H1_OA) + k2_OA*(eye(7,7) - gen_J_pinv*J)*transpose(now_g_H2_OA);
    NSP(:,i)=[gen_J_pinv*x_dot; k0_JL*(eye(7,7) - gen_J_pinv*J)*transpose(now_g_H0_JL); k1_OA*(eye(7,7) - gen_J_pinv*J)*transpose(now_g_H1_OA); k2_OA*(eye(7,7) - gen_J_pinv*J)*transpose(now_g_H2_OA)];
    ...
    % Update H, g_H of "OA"
    H1_OA = performance_criterion_OA(sepDist(:,1));
    now_g_H1_OA = get_gradient_H_OA(worldCollisionArray(1), WLN_q(:,i-1), WLN_q(:,i));
    H2_OA = performance_criterion_OA(sepDist(:,2));
    now_g_H2_OA = get_gradient_H_OA(worldCollisionArray(2), WLN_q(:,i-1), WLN_q(:,i));
...
function g_H = get_gradient_H_OA(worldCollisionArray, past_q, now_q)
    ...
    sepDist_past = check_nan(sepDist_past);
    sepDist_H1 = check_nan(sepDist_H1);
    sepDist_H2 = check_nan(sepDist_H2);
    sepDist_H3 = check_nan(sepDist_H3);
    sepDist_H4 = check_nan(sepDist_H4);
    sepDist_H5 = check_nan(sepDist_H5);
    sepDist_H6 = check_nan(sepDist_H6);
    sepDist_H7 = check_nan(sepDist_H7);
...
function update_array = check_nan(array)
    size_array = size(array,1);
    if isempty(isnan(array)) == false
        for i=1:size_array
            if isnan(array(i,1)) == true
                array(i,1) = 0;
            end
        end
    end
